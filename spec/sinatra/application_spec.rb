  ...
  context 'with avatars' do
    ...
    [['named color', 'red'], ['hex value', '7569af']].each do |type, color|
      context "#{type} (#{color})" do
        ...
        it 'returns the correct ETag' do
    ...
    context 'with invalid color (blu)' do
      let(:color) { 'blu' }
      ...
      xit 'matches error image' do
        expect(Digest::MD5.hexdigest(last_response.body)).to eq Digest::MD5.hexdigest(File.binread(File.expand_path('../examples/black/400/jr.png', __dir__)))
      end
    end
  end
end
